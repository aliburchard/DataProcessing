...
jdata_unfiltered <-read.csv(file = "../Data_Aggregation_CamCAT/data/se.angola.classifications_Ali.csv", header = F, stringsAsFactors = F)
names_file <- read.csv("data/chicago-wildlife-watch-classifications.csv", stringsAsFactors = F)
names(jdata_unfiltered) <- names(names_file)


# So, need to limit to final workflow version and ideally split by task. 
jdata_unfiltered %>% mutate(., created_at = ymdhms(created_at)) %>% 
     group_by(., workflow_id, workflow_version) %>% summarise(., max(created_at), n()) %>% View

# Run things on filtered and unfilterd to compare
jdata <- jdata_unfiltered #%>% filter(., workflow_version == set_workflow_version)
