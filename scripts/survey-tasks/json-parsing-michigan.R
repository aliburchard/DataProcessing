...
# This projects has both FOLLOW UP QUESTIONS as well as a "check all that apply" formatted subquestion, meaning that the classification data are a bit different thatn camera catalogue or chicago.
...
jdata <- jdata_unfiltered %>% filter(., workflow_version == set_workflow_version) %>% head(., n = 20000)

write.csv(jdata, "data/michigan-zoomin-filtered.csv")
...
with_answers_list <- with_choices %>%  #annoyingly, there's no easy way to unlist these. hence the hoop jumping in the next block.
     ...
     spread_values(how_many = jstring("HOWMANYANIMALSDOYOUSEE"), behavior = jstring("WHATISTHEANIMALSDOING"))
...
write.csv(flat_data, file = "data/jdata-flattened.csv")
